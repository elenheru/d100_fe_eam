subroutine place_atoms_bcc_cylindric(lp)
    ...
    ! start periodic placing
    first_periodic = last_relaxable + 1
    last_periodic = first_periodic
    do i=-iw, iw
    do j=-iw, iw
    do k=-iz, iz
        atom_position(1) = i * lp
        ...
        zone_geometry_condition = &
            zone_geometry_condition_periodic(atom_position)

        if ( zone_geometry_condition ) then
            R_at(:, last_periodic) = atom_position(:)
            last_periodic = last_periodic + 1
        endif
    ...
    enddo
    last_periodic = last_periodic - 1
    ! end periodic placing

    ! start casing placing
    first_casing = last_periodic + 1
    ...
    contains
        ...
        logical function zone_geometry_condition_periodic(r)
            real(float_byte_size), intent(in) :: r(3)
            zone_geometry_condition_periodic = (&
            ( norm2(r(1:2)) .lt. r_main_cell ) .and. &
            ( ( abs(r(3)) .gt. h_main_cell ) .and. &
            ( abs(r(3)) .lt. h_main_cell + dh_periodic_zone) ) )
        endfunction
        ...
        logical function zone_geometry_condition_casing(r)
            ...
            ( abs(r(3)) .lt. h_main_cell + dh_periodic_zone + dh_casing_zone) )&
            .and. (( (.not. zone_geometry_condition_periodic(r)) )&
        ...
        logical function zone_geometry_condition_wall(r)
            ...
            ( abs(r(3)) .lt. &
            h_main_cell + dh_periodic_zone + dh_casing_zone + dh_elastic_zone))&
            .and. ( ( (.not. zone_geometry_condition_relaxable(r)) )&
            .and. ( (.not. zone_geometry_condition_periodic(r)) &
