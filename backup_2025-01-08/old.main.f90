...
subroutine      spawn_bcc_rectangular_100
    ...
    real(float_byte_size) a_half
    integer(integer_byte_size) cnt_x, cnt_y, cnt_z, cnt_q
...
subroutine      part_three_zones_cylinder !find indexes for main comp cell and
    ...
    use phys_parameters_mod
    use anisotropy_mod
    real(float_byte_size) :: R_swap(1:3, atoms_max_array)!swap massive
    real(float_byte_size) :: xedge, yedge, zedge!explicit values for geometry conditions check
    real(float_byte_size), parameter :: okr=1d-3           !small epsilon
    integer(integer_byte_size) i, s
    !variables end
...
subroutine      cat_symmetric_pairs
    ...
    integer(integer_byte_size) i, j, s
    real(float_byte_size), parameter :: okr=1d-3           !small epsilon
...
!    INTEGER :: I_ATOMS
    real(float_byte_size) dist, vector_dist(3)
    verlet_list = 0
    distan_list = 0d0
...
subroutine      renew_verlet_list_atom(na) !particular
    ...
    integer(integer_byte_size) i, j, na
    real(float_byte_size) dist, vector_dist(3)
...
subroutine      cast_hirth_shifts
    ...
    real(float_byte_size), parameter :: okr=1d-3           !small epsilon
    real(float_byte_size) x, y, z, ux, uy, uz
...
subroutine      proceed_core_relaxation
    ...
    use comp_parameters_mod
    integer(integer_byte_size) i_passages, i_atoms
...
subroutine      set_positions_of_periodic_atoms
    use periodic_conditions_mod
    use positions_mod
    integer(integer_byte_size) i_couple, periodic_atom, body_atom
...
subroutine      energygradient_pw(na, dux, duy, duz) !this is antigradient
    use positions_mod
    use interaction_mod
    integer(integer_byte_size) i, k, na

    real(float_byte_size) delX, delY, delZ, dux, duy, duz, r!, SCALING
    real(float_byte_size) cos_alp, cos_bet, cos_gam
    real(float_byte_size) JPotOrig_d1, dpoti
...
subroutine      relax_atom(na)
    use positions_mod
    use phys_parameters_mod
    integer(integer_byte_size) na, i_denominate, i_step, i_retries
    integer(integer_byte_size), parameter :: max_denominate = 11, max_moves = 30, max_retries=4
    real(float_byte_size), parameter :: small_move = 8d-06, denominator =125d-3 != 8d-07
    !small_move = 5d-07 gives about 13 correct signs
    real(float_byte_size)     fx, fy, fz, xsh, ysh, zsh, normal, step
!    real(float_byte_size)     x_prev, y_prev, z_prev, energy_best
    real(float_byte_size)     energy_current, enew, eatom, rndm(max_retries+max_moves)
...
subroutine      conjugated_relaxation
    ...
    integer(integer_byte_size) i_passages, i_atoms, i_directions, grad_max_index, cgm_direction_multiplicator, thro
    real(float_byte_size) backstep_value(3000)
    real(float_byte_size) gamma_curr, gamma_prev, gamma_sum
    real(float_byte_size) scalar_curr, scalar_prev, scalar_summ
    real(float_byte_size) e_curr, e_prev, D_denominator, shift_magnitude
    real(float_byte_size) fx, fy, fz, grad_curr, maxgrad
    logical need_to_rescale_total_gradient, relax_the_max
!real(float_byte_size), dimension(1:3, atoms_max_array) :: F_prev, F_curr, D_prev, D_curr gamma_
...
subroutine      proceed_convergation_burgers_poisson
    ...
    !; cycle
    integer(integer_byte_size) i_anisotropy
...
subroutine      energy_of_system(e_sys)
    ...
    use positions_mod
    use interaction_mod
    real(float_byte_size), intent(out) :: e_sys
    real(float_byte_size) esum, JPotOrig, dist
...
subroutine      cat_matrixes
    ...
    integer(integer_byte_size) our_plane((2*x_layers+1)*(2*y_layers+1)*(2)), atoms_in_plane
    integer(integer_byte_size) i, s, i_x, i_y
    real(float_byte_size) z_const, y_const, x_const
...
subroutine      get_burgers_poisson_modified !# 110
    ...
    integer(integer_byte_size) i, j, na, nb
    real(float_byte_size) ux, uy, uz, x, y, z, phi_1, phi_2, burgers_temp, poisson_temp
    real(float_byte_size) alpha, beta, gamma_, c_, d_
    !integer(integer_byte_size) :: layer_list(atoms_max_array/4)
    !integer(integer_byte_size) :: max_layer_list_index
...
subroutine      wo_xyz_snapshot !#104
    ...
    use comp_parameters_mod
    USE phys_parameters_mod, ONLY:A0
    integer(integer_byte_size) i_atoms, S, w
...
subroutine      wo_summary_and_results(e_sys_init)!#133
    ...
    real(float_byte_size) e_sys, e_sys_init
...
subroutine      wo_matrixes_100 !## 221:229
    ...
    use matrixes_mod
    use comp_parameters_mod
    integer(integer_byte_size) i_x, i_y, na, n_left, n_rigt
...
subroutine      set_seed_of_random_numbers
    integer(integer_byte_size) date_values(8), feed_input(100)
...
integer(integer_byte_size) function centeral_atom_number()
    !returns number of atom that is most close to orgrin(x=0, y=0, z=0)
    use positions_mod
    use comp_parameters_mod

    integer(integer_byte_size) i, challenger
    real(float_byte_size) dist, dist_min
    challenger = 1
...
real(float_byte_size) function eatom(na)
    use positions_mod
    use interaction_mod

    integer(integer_byte_size) i, j, na
    real(float_byte_size) sum_pw, dist, JPotOrig
...
real(float_byte_size) pure function JPotOrig(r)   !Johnson's potential, ->[A], <-[eV]
...
real(float_byte_size) pure function JPotOrig_d1(r)   !Johnson's potential deriv1, ->[A], <-[eV/A]
    real(float_byte_size), parameter :: &
    A1=-2.195976d0, A2=3.097910d0, A3=2.704060d0, &!A4=7.436448d0, &
    B1=-0.639230d0, B2=3.115829d0, B3=0.477871d0, &!B4=1.581570d0, &
    C1=-1.115035d0, C2=3.066403d0, C3=0.466892d0   !C4=1.547967d0
...
subroutine      conjugated_relaxation2
    ...
    integer(integer_byte_size) i_passages, i_atoms, i_directions, grad_max_index, cgm_direction_multiplicator
    real(float_byte_size) gamma_curr, gamma_prev, gamma_sum
    real(float_byte_size) scalar_curr, scalar_prev, scalar_summ
    real(float_byte_size) e_curr, e_prev, D_denominator, shift_magnitude
    real(float_byte_size) fx, fy, fz, grad_curr, maxgrad
...
subroutine      cast_gusev_shifts_anisotropic
    ...
    use positions_mod
    use comp_parameters_mod
    integer(integer_byte_size) i, i_beauty
    real(float_byte_size), parameter :: okr=1d-3           !small epsilon
    real(float_byte_size) x, y, z, ux, uy, uz, phi_1, bv, nu
    if(core_sign.gt.0)then
...
subroutine      interpolated_burgers_poisson(angle, bv, nu)
    ...
    use comp_parameters_mod
    use phys_parameters_mod
    real(float_byte_size), intent(in ) :: angle
    real(float_byte_size), intent(out) :: bv, nu
    real(float_byte_size) phi_a, phi_b, burg_a, burg_b, pois_a, pois_b
    integer(integer_byte_size) i, j, na, nb
