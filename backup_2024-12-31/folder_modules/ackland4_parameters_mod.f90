module ackland4_parameters_mod
    save
    real(8), parameter, dimension(2) :: rsm_ack4pot = (/ 0.9, 1.95/)
    real(8), parameter, dimension(0:3) :: bbi_ack4pot = &
        (/ 14.996917289290, -20.533174190155, 14.002591780752, -3.6473736591143/)
    real(8), parameter, dimension(1:15) :: rphi_ack4pot = &
        (/ 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 3.0, 3.3, 3.7, 4.2, 4.7, 5.3, 6.0/)
    real(8), parameter, dimension(1:15) :: aphi_ack4pot = &
    ...
    real(8), parameter, dimension(1:3) :: rpsi_ack4pot = (/ 2.4, 3.2, 4.2/)
    real(8), parameter, dimension(1:3) :: apsi_ack4pot = &
        (/  11.686859407970,   -0.014710740098830, 0.47193527075943 /)
    real(8), parameter :: aphibi_ack4pot = -0.00034906178363530

    contains
        real(8) function varphi_ack4pot(x)
            implicit none
            real(8), intent(in) :: x
            varphi_ack4pot = 0.1818 * exp( -3.2000 * x) + 0.5099 * exp(-0.9423 * x) &
                           + 0.2802 * exp( -0.4029 * x) + 0.02817* exp(-0.2016 * x)
        endfunction
        real(8) function d1varphi_ack4pot(x)
            implicit none
            real(8), intent(in) :: x
