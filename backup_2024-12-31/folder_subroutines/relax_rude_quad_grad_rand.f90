subroutine relax_rude_quad_grad_rand(na,h)
    use positions_mod
    implicit none
    integer, intent(inout) :: na
    real(8), intent(inout) :: h
    real(8),parameter :: leapover_shrink = 4d0
    integer i
    real(8) :: e000, ep00, em00, e0p0, e0m0, e00p, e00m, esss, enorm, eg, eran
    real(8) :: xadd, yadd, zadd, addnorm, Rs(3), Rmin(3),R_ran(3)
    ...
    call calculate_energy_at(e000,na)
    esss = e000

    ! x addition
    r_at(1,na) = r_at(1,na) + h
    call calculate_energy_at(ep00,na)
    ...
    call calculate_energy_at(em00,na)
    ...
    call calculate_energy_at(e0p0,na)
    ...
    call calculate_energy_at(e0m0,na)
    ...
    call calculate_energy_at(e00p,na)
    ...
    call calculate_energy_at(e00m,na)
...
!        print*, "gradien minimization is not successful also. we'll try quadratic via gradient"
    ...
    r_at(2,na) = rs(2) + yadd
    r_at(3,na) = rs(3) + zadd
    call calculate_energy_at(eg,na)
    ...
    do i=1,10
        ...
        r_at(1:3,na) = r_at(1:3,na) + R_ran(1:3)
        call calculate_energy_at(eran,na)
...
endsubroutine relax_rude_quad_grad_rand
